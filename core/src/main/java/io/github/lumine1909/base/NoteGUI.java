...
public class NoteGUI implements BaseGUI {
    ...
    public NoteGUI() {
        ...
        inv.setItem(0, ItemBuilder.init(Material.NOTE_BLOCK, 1).name("&b音符盒 (0, " + snote(0) + ")").build());
        for (int i=1; i<25; i++) {
            inv.setItem(i, ItemBuilder.init(Material.NOTE_BLOCK, i).name("&b音符盒 (" + i + ", " +  snote(i) + ")").build());
        }
    }
    @Override
